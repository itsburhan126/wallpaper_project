...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('admin.categories.index') }}" class="px-4 py-2 bg-white border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50 transition-colors text-sm font-medium">
    ...
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div class="grid grid-cols-1 gap-6">
                <!-- Name -->
                <div>
                    <label for="name" class="block text-sm font-medium text-slate-700 mb-1">Category Name <span class="text-red-500">*</span></label>
                ...
                <div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 {{ $category->image ? '' : 'border-dashed' }} border-slate-300 rounded-lg hover:border-indigo-500 transition-colors relative" id="drop-zone">
                        <div class="space-y-1 text-center {{ $category->image ? 'hidden' : '' }}" id="upload-prompt">
                        ...
                        <div id="image-preview" class="{{ $category->image ? '' : 'hidden' }} absolute inset-0 flex items-center justify-center bg-white rounded-lg">
                            <img src="{{ $category->image ? asset($category->image) : '' }}" class="max-h-full max-w-full rounded-lg object-contain p-2">
                ...
                <div class="flex items-center">
                    <div class="flex items-center h-5">
                        <input id="status" name="status" type="checkbox" value="1" {{ $category->status ? 'checked' : '' }}
                    ...
                    <div class="ml-3 text-sm">
                        ...
                        <p class="text-slate-500">Enable this category for users</p>
                    </div>
                </div>
            </div>

            <div class="mt-8 pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
                <a href="{{ route('admin.categories.index') }}" class="px-4 py-2 bg-white border border-slate-200 rounded-lg text-slate-700 hover:bg-slate-50 transition-colors text-sm font-medium">
                    Cancel
                </a>
