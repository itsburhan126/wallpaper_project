        ...
        <form action="{{ route('admin.roles.update', $role->id) }}" method="POST">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
                <div class="p-6 border-b border-slate-100 bg-slate-50/50">
                    <h2 class="text-lg font-bold text-slate-800">Edit Role: {{ $role->name }}</h2>
                ...
                <div class="p-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <label class="relative flex items-start p-4 rounded-lg border border-slate-200 cursor-pointer hover:bg-indigo-50 hover:border-indigo-200 transition-all group {{ in_array($key, $role->permissions ?? []) ? 'bg-indigo-50 border-indigo-200' : '' }}">
                ...
                <div class="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2.5 rounded-lg bg-indigo-600 text-white font-medium hover:bg-indigo-700 shadow-sm shadow-indigo-200 transition-all">Update Role</button>
                </div>
            </div>
        </form>
    </div>
