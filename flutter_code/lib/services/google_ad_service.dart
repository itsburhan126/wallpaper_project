...
class GoogleAdService {
  ...
  Future<bool> loadRewardedAd(BuildContext context, {bool force = false}) async {
    ...
    if (!adProvider.adsEnabled || adProvider.admobRewardedId.isEmpty) {
      debugPrint("⚠️ Ads disabled or ID missing in loadRewardedAd");
      return false;
    }

    // Check if a load is already in progress
    if (!force && _rewardedAdLoadFuture != null) {
        debugPrint("⏳ Ad load already in progress, returning existing future");
        return _rewardedAdLoadFuture!;
    }

    if (force) {
      debugPrint("💪 Forcing new ad load...");
      _rewardedAdLoadFuture = null; // Clear existing future
    }

    debugPrint("🎬 Loading Rewarded Ad: ${adProvider.admobRewardedId}");
    ...
    Timer(const Duration(seconds: 40), () {
      if (!completer.isCompleted) {
        debugPrint("⚠️ Rewarded Ad Load Timed Out (Internal 40s limit) - Resetting state");
  ...
  Future<bool> showRewardedAd(BuildContext context, {required Function(int) onReward, required VoidCallback onFailure}) async {
    ...
    debugPrint("⚙️ Ad Provider Setting: ${adProvider.adProvider}");

    // Check if only AdMob is allowed
    if (adProvider.adProvider != 'admob_only' && adProvider.adProvider != 'both') {
      debugPrint("⚠️ AdMob is disabled by admin settings (Provider: ${adProvider.adProvider})");
      onFailure();
      return false;
    }

    if (_rewardedAd == null) {
        debugPrint("❌ Rewarded Ad is null in showRewardedAd (and we should have checked readiness before)");
        onFailure();
        return false;
    } else {
      debugPrint("✅ _rewardedAd is NOT NULL, proceeding to show");
    }
    ...
    try {
      debugPrint("👉 invoking _rewardedAd!.show()");
      ...
      debugPrint("💥 Exception showing ad: $e");
      onFailure();
      return false;
    }
  ...
  Future<bool> loadInterstitialAd(BuildContext context) async {
    ...
    debugPrint("🎬 Loading Interstitial Ad: ${adProvider.admobInterstitialId}");
  ...
  Future<bool> showInterstitialAd(BuildContext context, {VoidCallback? onAdDismissed}) async {
    ...
    _interstitialAd!.fullScreenContentCallback = FullScreenContentCallback(
      ...
      onAdShowedFullScreenContent: (ad) {
        debugPrint("🎬 Interstitial Ad showed fullscreen");
      },
    );
